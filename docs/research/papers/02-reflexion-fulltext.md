# Reflexion: Language Agents with Verbal Reinforcement Learning (全文抓取)

> 来源: https://arxiv.org/html/2303.11366 (arXiv HTML) | 抓取日期: 2026-08-11
...
Recent works such as ReAct Yao et al., (2023), SayCan Ahn et al., (2022),
Toolformer Schick et al., (2023), HuggingGPT (Shen et al.,, 2023),
generative agents (Park et al.,, 2023), and WebGPT (Nakano et al.,, 2021)
...
Generating useful reflective feedback is challenging since it requires a good understanding of where the model made mistakes (i.e. the credit assignment problem (Sutton and Barto,, 2018)) as well as the ability to generate a summary containing actionable insights for improvement. We explore three ways for
...
Reflexion agents improve on decision-making AlfWorld (Shridhar et al.,, 2021) tasks over strong baseline approaches by
an absolute 22% in 12 iterative learning steps, and
on reasoning questions in HotPotQA (Yang et al.,, 2018) by 20%, and Python programming
tasks on HumanEval (Chen et al.,, 2021) by as much as 11%.
...
Figure 1: Reflexion works on decision-making 4.1,
programming 4.3, and reasoning 4.2 tasks.

## 2 Related work

##### Reasoning and decision-making

Self-Refine (Madaan et al.,, 2023) employs an iterative framework for
...
but is limited to single-generation reasoning tasks. Pryzant et al., (2023)
performs a similar semantic prompt-writing optimization, but is
also limited to single-generation tasks.
Paul et al., (2023) fine-tune critic models to provide intermediate
feedback within trajectories to improve reasoning responses.
Xie et al., (2023) use stochastic beam search over actions to
...
Yoran et al., (2023) and Nair et al., (2023) use decider models
to reason over several generations. Kim et al., (2023) use a
...
or code debugging practices. AlphaCode (Li et al.,, 2022)
evaluates a set of generations on hidden test cases. CodeT (Chen et al.,, 2022)
uses self-generated unit tests that are used to score generated function
implementations.
Self-Debugging (Chen et al.,, 2023) employs a debugging component that is used
to improve existing implementations given feedback from a code
execution environment. CodeRL (Le et al.,, 2022) sets the problem
...
$subscript𝑀𝑎M_{a}$ ,  $subscript𝑀𝑒M_{e}$ ,  $subscript𝑀𝑠𝑟M_{sr}$

Initialize policy  $subscript𝜋𝜃conditionalsubscript𝑎𝑖subscript𝑠𝑖\pi_{\theta}(a_{i}|s_{i})$ ,  $𝜃subscript𝑀𝑎𝑚𝑒𝑚\theta=\{M_{a},mem\}$

Generate initial trajectory using  $subscript𝜋𝜃\pi_{\theta}$

Evaluate  $subscript𝜏0\tau_{0}$  using  $subscript𝑀𝑒M_{e}$

Generate initial self-reflection  $𝑠subscript𝑟0sr_{0}$  using  $subscript𝑀𝑠𝑟M_{sr}$

Set  $←𝑚𝑒𝑚delimited-[]𝑠subscript𝑟0mem\leftarrow[sr_{0}]$

Set  $𝑡0t=0$

while  $subscript𝑀𝑒M_{e}$  not pass or  $𝑡absenttdo

     Generate  $subscript𝜏𝑡
subscript𝑎0subscript𝑜0…subscript𝑎𝑖subscript𝑜𝑖\tau_{t}={[a_{0},o_{0},\dots a_{i},o_{i}]}$  using  $subscript𝜋𝜃\pi_{\theta}$

     Evaluate  $subscript𝜏𝑡\tau_{t}$  using  $subscript𝑀𝑒M_{e}$

     Generate self-reflection  $𝑠subscript𝑟𝑡sr_{t}$  using  $subscript𝑀𝑠𝑟M_{sr}$

     Append  $𝑠subscript𝑟𝑡sr_{t}$  to  $𝑚𝑒𝑚mem$
...
an Actor, denoted as  $subscript𝑀𝑎M_{a}$ , which generates text and actions;
an Evaluator model, represented by  $subscript𝑀𝑒M_{e}$ , that scores the outputs produced
by  $subscript𝑀𝑎M_{a}$ ; and a Self-Reflection model, denoted as  $subscript𝑀𝑠𝑟M_{sr}$ ,
...
or generation,  $subscript𝑎𝑡a_{t}$ , from the current policy  $subscript𝜋𝜃\pi_{\theta}$  at time  $𝑡t$ , receive
an observation from the environment  $subscript𝑜𝑡o_{t}$ . We explore various Actor models,
including Chain of Thought Wei et al., (2022) and ReAct Yao et al., (2023).
...
was inspired by Brooks et al., (2022), who suggest a policy iteration approach using in-context learning. Details on how this
...
in the agent’s memory (mem). For instance, in a multi-step decision-making
task, when the agent receives a failure signal, it can infer that a specific
action  $subscript𝑎𝑖a_{i}$  led to subsequent incorrect actions  $subscript𝑎
𝑖1a_{i+1}$  and  $subscript𝑎
𝑖2a_{i+2}$ . The
agent can then verbally state that it should have taken a different action,
$superscriptsubscript𝑎𝑖′a_{i}^{\prime}$ , which would have resulted in  $superscriptsubscript𝑎
𝑖1′a_{i+1}^{\prime}$  and  $superscriptsubscript𝑎
𝑖2′a_{i+2}^{\prime}$ , and store
this experience in its memory. In subsequent trials, the agent can leverage
its past experiences to adapt its decision-making approach at time  $𝑡t$  by
choosing action  $superscriptsubscript𝑎𝑖′a_{i}^{\prime}$ . This iterative process of trial, error, self-reflection,
...
In the first trial, the Actor produces a trajectory  $subscript𝜏0\tau_{0}$  by
interacting with the environment. The Evaluator then produces a score  $subscript𝑟0r_{0}$
which is computed as  $subscript𝑟𝑡subscript𝑀𝑒subscript𝜏0r_{t}=M_{e}(\tau_{0})$ .  $subscript𝑟𝑡r_{t}$  is only a scalar
reward for trial  $𝑡t$  that improves as task-specific performance increases.
After the first trial, to amplify  $subscript𝑟0r_{0}$  to a feedback form that can be used
for improvement by an LLM, the Self-Reflection
model analyzes the set of  $subscript𝜏0subscript𝑟0\{\tau_{0},r_{0}\}$  to produce a summary
$𝑠subscript𝑟0sr_{0}$  which is stored in the memory mem.  $𝑠subscript𝑟𝑡sr_{t}$  is a verbal
experience feedback for trial  $𝑡t$ . The Actor, Evaluator, and Self-Reflection
models work together through trials in a loop until the Evaluator deems  $subscript𝜏𝑡\tau_{t}$  to
be correct. As mentioned in 3, the memory component of
Reflexion is crucial to its effectiveness. After each trial  $𝑡t$ ,  $𝑠subscript𝑟𝑡sr_{t}$ , is appended
mem. In practice, we bound mem by a maximum number of stored experiences,  $Ω\Omega$
...
search-based question answering on HotPotQA (Yang et al.,, 2018), multi-step
tasks in common household environments in AlfWorld (Shridhar et al.,, 2021), and code
writing tasks in competition-like environments with interpreters and
compilers in HumanEval (Chen et al.,, 2021), MBPP (Austin et al.,, 2021),
...
(Côté et al.,, 2019). Following Yao et al., (2023), we run the agent in
...
chilling a tomato in the fridge). We use ReAct (Yao et al.,, 2023) as the
action generator as Yao et al., (2023) has shown success in long trajectory
...
trajectory examples as Yao et al., (2023) with GPT-3 for the LLM. AlfWorld
...
HotPotQA (Yang et al.,, 2018) is a Wikipedia-based dataset with 113k
...
+ Chain-of-Thought (CoT) (Wei et al.,, 2022) for step-by-step  $→𝑄𝐴Q\rightarrow A$  and
$𝑄Q$ ,  $→subscript𝐶𝑔𝑡𝐴C_{gt}\rightarrow A$  implementations, where  $𝑄Q$  is the question,  $subscript𝐶𝑔𝑡C_{gt}$
is the ground truth context from the dataset, and  $𝐴A$  is the final answer.
Since CoT is not a multi-step
decision-making technique, we give  $subscript𝐶𝑔𝑡C_{gt}$  to the agent so that we can isolate
...
implement a Reflexion + ReAct (Yao et al.,, 2023) agent that can retrieve
...
writing on MBPP (Austin et al.,, 2021), HumanEval (Chen et al.,, 2021),
and LeetcodeHardGym, our new dataset. MBPP and HumanEval measure function body
generation accuracy given natural language
descriptions. We use a benchmark language compiler, MultiPL-E (Cassano et al.,, 2022),
...
sample  $𝑛n$  tests from the collection of generated unit tests to produce a
test suite  $𝑇T$ , denoted as  $subscript𝑡0subscript𝑡1…subscript𝑡𝑛\{t_{0},t_{1},\dots,t_{n}\}$ . We set  $𝑛n$  to a maximum
...
| HumanEval (PY)
| 65.8 (CodeT (Chen et al.,, 2022) + GPT-3.5)
| 80.1 (GPT-4)
| 91.0
...
| MBPP (PY)
| 67.7 (CodeT (Chen et al.,, 2022) + Codex (Chen et al.,, 2021))
|
80.1 (GPT-4)
| 77.1
...
translated to Rust with MultiPL-E (Cassano et al.,, 2022). TP: unit
...
solution and lead to a false positive label on a code completion (Lam et al.,, 2020).
...
arXiv preprint arXiv:2210.03821.
...
In Computer Games: 7th Workshop, CGW 2018, Held in Conjunction
with the 27th International Conference on Artificial Intelligence, IJCAI
2018, Stockholm, Sweden, July 13, 2018, Revised Selected Papers 7, pages
41–75. Springer.
...
Environment: You are in the middle of a room. Looking quickly
...
shelf 1.Task: examine the mug with the desklamp.
...
$>$  go to desk 1On the desk 1, you see a creditcard 3, a desklamp 1, a laptop 2, a mug 1, a
pen 1, and a pencil 1. $>$  think: Now I find a mug (1). Next, I need to take it. $>$  take mug 1 from desk 1You pick up the mug 1 from the desk 1.
...
$>$  use desklamp 1Nothing happens. $>$  use desklamp 1Nothing happens.Status: Fail
...
Trial #2
$>$  go to desk 1On the desk 1, you see a creditcard 3, a desklamp 1, a laptop 2, a mug 1,
a pen 1, and a pencil 1. $>$  think: To solve the task, I need to find and take a mug, then find
and use a desklamp. $>$  take mug 1 from desk 1You pick up the mug 1 from the desk 1.
$>$  think: To solve the task, I need to find and take a mug, then find
and use a desklamp.
$>$  use desklamp 1You turn on the desklamp 1.Status: Success

Figure 5: [Top] An AlfWorld trajectory in which the agent failed due to
...
shortcoming in an experiment on WebShop (Yao et al., rint, ). WebShop is a web-based
...
Figure 6: Reflexion vs React performance on WebShop across 100 customer
...
Trial #1: Question: Grown-Ups starred the actor who was
...
1980.
Thought 2: The paragraph does not mention the actor who
...
Reflection: I searched the wrong title for the
...
Figure 7: Two HotPotQA trials within the same environment and task.
