...
try:
    with db.get_session() as s:
        fts_hits = set(
            search_fts(
                s.connection().connection.driver_connection, query, top_k=settings.FTS_RECALL_TOP_K
            )
        )
except Exception:
...
for i, m in enumerate(items):
    ...
    score = (
        settings.RETRIEVAL_W_VECTOR * vs
...
if extra_ids:
    with db.get_session() as s:
        extra = s.exec(
            select(MemoryItem).where(MemoryItem.id.in_(extra_ids), MemoryItem.status == "active")
        ).all()
    for m in extra:
        scored_items.append(
...
def test_hybrid_fts_extra_recall(engine):
    ...
    with (
        ...
        patch(
            "remembrance.retrieval.hybrid.get_vector_store", return_value=fake_store([vec_hit_id])
        ),
    ):
