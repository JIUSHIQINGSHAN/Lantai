...
def collect_observation_status(
    ...
    rows = conn.execute(
        ...
        "FROM reflect_run ORDER BY run_at DESC"
    ).fetchall()
...
def main(argv: list[str] | None = None, database_path: Path | None = None) -> int:
    ...
    parser.add_argument(
        "--required", type=int, default=None, help="窗口内合格天数门槛（默认 settings）"
    )
    parser.add_argument(
        "--window", type=int, default=None, help="滑动窗口大小（天，默认 settings）"
    )
    args = parser.parse_args(argv)

    required = (
        args.required if args.required is not None else settings.REFLECT_OBSERVATION_REQUIRED_RUNS
    )
