...
def test_render_scene_page_shape():
    ...
    members = [
        MemoryItem(
            id="m1", memory_type="semantic", key="k1", content="用户喜欢 Python", lane="preference"
        ),
        MemoryItem(
            id="m2", memory_type="semantic", key="k2", content="部署用 systemd", lane="fact"
        ),
    ]
...
def test_run_wiki_removes_stale_pages(mem_db, monkeypatch, tmp_path):
    ...
    with session_factory() as s:
        s.add(
            MemoryScene(
                id="s1", name="旧场景", summary="", heat=1, member_count=0, centroid=[1.0, 0.0, 0.0]
            )
        )
        s.commit()
    monkeypatch.setattr(ws.settings, "WIKI_OUTPUT_DIR", str(tmp_path))
    ...
    with session_factory() as s:
        s.add(
            MemoryScene(
                id="s2", name="新场景", summary="", heat=2, member_count=0, centroid=[1.0, 0.0, 0.0]
            )
        )
...
def test_run_wiki_llm_overview_uses_llm(mem_db, monkeypatch, tmp_path):
    ...
    with patch(
        "lantai.llm.client.chat_json", return_value={"overview": "LLM 叙事综述 [[场景甲]]"}
    ) as m:
        res = ws.run_wiki_update_once()
...
def test_run_wiki_llm_failure_falls_back(mem_db, monkeypatch, tmp_path):
    ...
    with patch("lantai.llm.client.chat_json", side_effect=RuntimeError("llm down")):
        res = ws.run_wiki_update_once()
    assert res["overview"] == "fallback"
    assert (tmp_path / "overview.md").exists()
...
def test_mcp_wiki_read(mem_db, monkeypatch, tmp_path):
    ...
    with session_factory() as s:
        s.add(
            MemoryScene(
                id="s1", name="部署", summary="s", heat=1, member_count=1, centroid=[1.0, 0.0, 0.0]
            )
        )
        s.add(
            MemoryItem(
    ...
    resp = mod.handle(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "wiki_read", "arguments": {"slug": "部署"}},
        }
    )
    ...
    resp3 = mod.handle(
        {
            ...
            "id": 3,
            "method": "tools/call",
            "params": {"name": "wiki_read", "arguments": {"slug": "不存在"}},
        }
    )
