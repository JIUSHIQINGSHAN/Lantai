...
def test_batch_reject_is_partial_and_real_db(param_env):
    ...
    result = batch_reject(
        BatchRejectRequest(
            reason="测试拒绝",
            items=[
                BatchItemRef(kind="candidate", source_id="cand-1"),
    ...
    with session_factory() as s:
        ...
        assert (
            s.exec(select(MemoryProposal).where(MemoryProposal.id == "prop-1")).first().status
            == "pending"
        )
