...
class TestReflectionDistribution:
    ...
    def test_reflection_window_consistency(self, digest_env):
        ...
        assert rf["other"] == 1
        assert rf["applied"] + rf["pending"] + rf["rejected"] + rf["other"] == rf["created"]
    ...
    def test_collect_calibration_stats_window(self, digest_env):
        ...
        _seed(
            ...
            [
                ...
                ReflectRun(id="run3", run_at=now, waterline=3.0, skipped="", proposals_created=0),
            ],
        )
    ...
    def test_render_calibration_markdown(self, digest_env):
        ...
        _seed(
            ...
            [
                ...
                ReflectRun(id="run1", run_at=now, waterline=5.5, skipped="", proposals_created=1),
            ],
        )
    ...
    def test_runs_five_way_classification(self, digest_env):
        ...
        _seed(
            ...
            [
                ...
                ReflectRun(id="r_llm", run_at=now, skipped="", curate_failed=True),
                ReflectRun(
                    id="r_llm2", run_at=now, skipped="", rejecter_failed=2, proposals_created=1
                ),
                ReflectRun(
                    id="r_prod", run_at=now, skipped="", proposals_created=2, auto_applied=2
                ),
    ...
    def test_window_counts(self, digest_env):
        ...
        _seed(
            ...
            [
                ...
                # 待审：今日新增 1 + 老待审 1
                MemoryCandidate(
                    id="cand_p1", document_id="doc1", status="pending_review", created_at=now
                ),
                MemoryCandidate(
                    id="cand_p2", document_id="doc2", status="pending_review", created_at=old
                ),
                # 归档：今日创建即归档 1，窗外 1
                MemoryCandidate(
                    id="cand_r1", document_id="doc3", status="rejected", created_at=now
                ),
                MemoryCandidate(
                    id="cand_r2", document_id="doc4", status="rejected", created_at=old
                ),
