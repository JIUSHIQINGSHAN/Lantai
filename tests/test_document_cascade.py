...
from lantai.models.tables import DocumentChunk, MemoryCandidate, MemoryEdge, MemoryItem, RawDocument
...
def test_document_cascade_delete(mem_db, client, monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: type("MockVS", (), {"delete": lambda self, ids: None})(),
    )
    ...
    with session_factory() as s:
        ...
        edge = MemoryEdge(
            id=new_id("edge"), source_memory_id=doc_id, target_memory_id=mem_id, relation="extract"
        )
        s.add(edge)
...
def test_document_cascade_delete_partial_memory(mem_db, client):
    ...
    with session_factory() as s:
        ...
        s.add(
            MemoryEdge(
                id="e1", source_memory_id=doc1_id, target_memory_id=mem_id, relation="extract"
            )
        )
        s.add(
            MemoryEdge(
                id="e2", source_memory_id=doc2_id, target_memory_id=mem_id, relation="extract"
            )
        )
        s.commit()
