...
def _proposal(**kw):
    defaults = dict(
        ...
        candidate_id=None,
        evidence_ids=["doc_1"],
        proposed_patch={"memory_type": "semantic", "key": "x", "content": "y", "lane": "general"},
        reason="distill",
        confidence=0.8,
        status="pending",
    )
...
class TestDecideProposal:
    ...
    def test_reject_persists_reason(self, review_env):
        ...
        res = decide_proposal("prop_r", ProposalDecisionReq(approve=False, reason="与现有记忆冲突"))
    ...
    def test_approve_persists_reason(self, review_env):
        ...
        with (
            ...
            res = decide_proposal("prop_a", ProposalDecisionReq(approve=True, reason="人工确认"))
        assert res["ok"] is True
        with session_factory() as s:
...
class TestReflectorAutoReject:
    ...
    def test_auto_reject_persists_verdict_reason(self, review_env):
        ...
        with patch("lantai.evolution.reflector.chat_json", side_effect=fake_chat_json):
