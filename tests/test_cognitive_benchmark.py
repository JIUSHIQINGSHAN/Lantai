...
def test_benchmark_memory_pollution_resilience(session, test_client):
    ...
    """
    for i in range(50):
        _insert_memory(session, f"The weather in city {i} is currently sunny with a temperature of {20 + i} degrees.")
    ...
    for i in range(50, 100):
        _insert_memory(session, f"User's favorite color in session {i} was blue.", role=CognitiveRole.BELIEF)

    results = hybrid_search(query="core engine Lantai FastAPI SQLite", 
        top_k=5, use_rerank=False, explain=True
    )
    
    assert len(results) > 0, "Failed to retrieve any memories"
    assert results[0]["memory"]["content"] == target_content, "Target memory is not at Rank 1 (Pollution failed)"
...
def test_benchmark_temporal_supersedes(session, test_client):
    ...
    """
    _insert_memory(session, "The CEO of the company is Alice.", role=CognitiveRole.OBSERVATION, created_at=utcnow() - timedelta(days=365))
    _insert_memory(session, "The CEO of the company is Bob.", role=CognitiveRole.OBSERVATION, created_at=utcnow())

    results = hybrid_search(query="CEO company", 
        top_k=5, use_rerank=False, explain=True
    )
    ...
    assert "Bob" in results[0]["memory"]["content"], "New fact (Bob) did not supersede old fact (Alice) at Rank 1"
...
def test_benchmark_lane_cognitive_dynamics(session, test_client):
    ...
    _insert_memory(session, "Always write tests before code (TDD).", role=CognitiveRole.RULE, created_at=old_time)
    _insert_memory(session, "I wrote some tests before coding today.", role=CognitiveRole.OBSERVATION, created_at=old_time)

    results = hybrid_search(query="write tests before code TDD", 
        top_k=5, use_rerank=False, explain=True
    )

    assert len(results) >= 2
    assert results[0]["memory"]["role"] == CognitiveRole.RULE, "Rule failed to persist stronger than General memory"
