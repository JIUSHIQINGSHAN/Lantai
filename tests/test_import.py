...
def test_parse_session_line_valid_and_invalid():
    ...
    msg = parse_session_line(
        json.dumps(
            {
                "role": "user",
                "content": "记住：我喜欢 Python",
    ...
    assistant = parse_session_line(
        json.dumps({"role": "assistant", "content": "好的", "timestamp": "2024-07-03T09:46:40Z"})
    )
    ...
    assert (
        parse_session_line(
            json.dumps({"role": "system", "content": "x", "timestamp": 1720000000000})
        )
        is None
    )
    assert (
        parse_session_line(json.dumps({"role": "user", "content": "x", "timestamp": "bad"})) is None
    )
    assert (
        parse_session_line(
            json.dumps(
                {"role": "user", "content": "x", "timestamp": 1720000000000, "session": "s1"}
            )
        )["session"]
        == "s1"
    )
...
def test_import_dry_run_no_writes(mem_db, tmp_path):
    ...
    f = tmp_path / "sessions.jsonl"
    f.write_text(
        json.dumps({"role": "user", "content": "记住：X", "timestamp": 1720000000000}),
        encoding="utf-8",
    )
    from lantai.ingestion.import_service import import_session_jsonl
...
def test_import_extract_path_uses_import_provenance(mem_db, tmp_path):
    ...
    with patch(
        "lantai.ingestion.dialogue.extract_candidate",
        return_value={
            "topic": ["t"],
            "summary": "s",
            "claims": [],
...
def test_import_bad_line_does_not_stop_batch(mem_db, tmp_path):
    ...
    f.write_text(
        "\n".join(
            [
                json.dumps(
                    {"role": "user", "content": "记住：第一条", "timestamp": 1720000000000},
                    ensure_ascii=False,
                ),
                "garbage",
                json.dumps(
                    {"role": "user", "content": "记住：第二条", "timestamp": 1720000001000},
                    ensure_ascii=False,
                ),
            ]
        ),
...
def test_chain_carries_import_timestamp_to_memory(mem_db):
    ...
    session_factory, _ = mem_db
    prov = {
        "prompt": "dialogue-session-import",
        "model": "test",
