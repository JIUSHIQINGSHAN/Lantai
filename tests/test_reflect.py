...
class TestHealthScan:
    def test_superseded_residual(self, reflect_env):
        ...
        assert any(
            c["memory_id"] == "mem_old" and c["signal"] == "superseded" for c in scan["candidates"]
        )
    ...
    def test_expired_window(self, reflect_env):
        ...
        _seed(
            session_factory,
            [_mem(id="mem_exp", content="一次性会议安排", valid_to=utcnow() - timedelta(days=1))],
        )
    ...
    def test_open_conflicts(self, reflect_env):
        ...
        _seed(
            ...
            [
                ...
                ConflictEvent(
                    id="cf_1",
                    memory_id="mem_cf",
                    incoming_ref="端口 9090",
                    rule_name="port-mutex",
                    kind="mutex",
                    status="open",
                ),
            ],
        ...
        assert any(
            c["signal"] == "open_conflict" and c["conflict_event_id"] == "cf_1"
...
class TestRunReflectOnce:
    ...
    def test_auto_apply_deprecate(self, reflect_env):
        ...
        with session_factory() as s:
            ...
            ckpts = s.exec(
                select(MemoryCheckpoint).where(MemoryCheckpoint.memory_id == "mem_old")
            ).all()
            assert len(ckpts) >= 1
    ...
    def test_pending_low_confidence(self, reflect_env):
        ...
        with session_factory() as s:
            prop = s.exec(select(MemoryProposal).where(MemoryProposal.proposal_type == "add")).one()
...
class TestRunOutcome:
    ...
    def test_curate_failure_records_flag(self, reflect_env):
        ...
        with (
            patch("lantai.evolution.reflector.chat_json", side_effect=RuntimeError("llm down")),
            _embed_mock(),
            _vector_mocks(),
        ):
            result = run_reflect_once()
    ...
    def test_rejecter_failure_records_flag(self, reflect_env):
        ...
        with (
            patch(
                "lantai.evolution.reflector.chat_json",
    ...
    def test_exception_records_waterline_and_error(self, reflect_env):
        ...
        _seed(
            session_factory,
            [
                _mem(id="mem_a", content="重要记忆", importance=0.8, created_at=utcnow()),
            ],
        )
        from lantai.evolution.reflector import run_reflect_once

        with (
            patch("lantai.evolution.reflector.health_scan", side_effect=RuntimeError("scan boom")),
        ...
        with session_factory() as s:
            ...
            edge = s.exec(
                select(MemoryEdge).where(
                    MemoryEdge.relation == "supersedes", MemoryEdge.target_memory_id == "mem_old"
                )
            ).one()
        ...
        with session_factory() as s:
            ...
            assert s.exec(
                select(MemoryEdge).where(
                    ...
                    MemoryEdge.target_memory_id == "mem_b",
                )
            ).one()
            assert (
                len(
                    s.exec(
                        select(MemoryCheckpoint).where(MemoryCheckpoint.memory_id == "mem_a")
                    ).all()
                )
                >= 1
            )
            assert (
                len(
                    s.exec(
                        select(MemoryCheckpoint).where(MemoryCheckpoint.memory_id == "mem_b")
                    ).all()
                )
                >= 1
            )
