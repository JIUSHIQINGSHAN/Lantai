...
PAPERS = [
    {
        ...
        "content": ("Dense and sparse fusion: vector weight 0.55 with BM25 0.30 improves recall."),
    },
    {
        ...
        "content": (
            "Recency weighting helps long-horizon agent recall, but "
...
def _sug(conf=0.9, evidence_docs=("doc_a",), after_vector=0.55):
    return {
        ...
        "changes": [
            {"name": "RETRIEVAL_W_VECTOR", "before": 0.6, "after": after_vector, "reason": "r"},
...
SNAP = {
    ...
    "RETRIEVAL_W_FTS": 0.05,
    "RETRIEVAL_W_DECAY": 0.1,
    "DEDUP_MERGE_THRESHOLD": 0.9,
    "DEDUP_UPDATE_THRESHOLD": 0.65,
}
...
class TestBatchValidation:
    def test_valid_batch_passes(self):
        result = validate_batch_advice(_batch(), SNAP, PAPERS, _views(doc_a={}))
    ...
    def test_all_tier_d_dropped(self):
        with pytest.raises(ParamValidationError):
            validate_batch_advice(
                _batch(), SNAP, PAPERS, _views(doc_a=dict(tier="D", eligible=False))
            )

    def test_low_confidence_fails(self):
        with pytest.raises(ParamValidationError):
            validate_batch_advice(
                _batch(suggestions=[_sug(conf=0.80)]), SNAP, PAPERS, _views(doc_a={})
            )
    ...
    def test_fake_contradiction_quote_drops_batch(self):
        c = {
            "param_key": "RETRIEVAL_W_DECAY",
            "nature": "direction",
            "side_a": {"source_document_id": "doc_a", "quote": "we prove vector 0.99 is best"},
            "side_b": {"source_document_id": "doc_b", "quote": "recency weighting helps"},
            "scope_note": "",
            "resolution": "report_to_human",
        }
        with pytest.raises(ParamValidationError):
            validate_batch_advice(
                _batch(contradictions=[c]), SNAP, PAPERS, _views(doc_a={}, doc_b={})
            )
    ...
    def test_contradiction_partition_blocks_param(self):
        ...
        c = {
            "param_key": "RETRIEVAL_W_DECAY",
            "nature": "direction",
            "side_a": {"source_document_id": "doc_a", "quote": "vector weight 0.55 with BM25 0.30"},
            "side_b": {
                "source_document_id": "doc_b",
                "quote": "aggressive decay discards stable facts",
            },
            "scope_note": "scope differs",
            "resolution": "report_to_human",
        }
        # 一条改 W_DECAY 的建议（会被拦）+ 一条正常建议
        bad_sug = _sug()
        bad_sug["changes"] = [
            {"name": "RETRIEVAL_W_DECAY", "before": 0.1, "after": 0.15, "reason": "r"}
        ]
        good_sug = _sug()
        ...
        assert len(result["suggestions"]) == 1
        assert result["suggestions"][0]["suggestion"].changes[0].name == "RETRIEVAL_W_VECTOR"
    ...
    def test_contradiction_same_source_fails(self):
        c = {
            ...
            "nature": "direction",
            "side_a": {"source_document_id": "doc_a", "quote": "vector weight 0.55 with BM25 0.30"},
            "side_b": {"source_document_id": "doc_a", "quote": "vector weight 0.55 with BM25 0.30"},
            "scope_note": "",
            "resolution": "report_to_human",
        }
        with pytest.raises(ParamValidationError):
...
class TestTierQuorum:
    ...
    def test_delta_budget_scaled_by_tier(self):
        ...
        with pytest.raises(ParamValidationError) as ei:
            validate_batch_advice(
                _batch(suggestions=[_sug(evidence_docs=("doc_a", "doc_b"), after_vector=0.5)]),
                SNAP,
                PAPERS,
                _views(doc_a=dict(tier="C"), doc_b=dict(tier="C")),
            )
    ...
    def test_scale_delta_budget_value(self):
        ...
        assert scale_delta_budget(Decimal("0.1"), Decimal("0.5")) == Decimal("0.05")
...
class TestWorkerBatch:
    def _seed(self, param_env, n=5):
        ...
        doc_ids = []
        for i in range(n):
            body = f"vector weight 0.55 with BM25 0.30 improves recall variant {i}"
            doc = RawDocument(
                id=new_id("doc"),
                source_type="paper",
                source_id=f"s{i}",
            ...
            upsert_from_draft(
                doc.id,
                QualitySignalDraft(arxiv_id=f"2503.{i}", journal_ref="Proc. SIGIR 2025", version=2),
            )
        return doc_ids
    ...
    def test_worker_batch_creates_suggestion_and_contradiction(self, param_env):
        ...
        with patch("lantai.parameters.advisor.chat_json", return_value=payload):
