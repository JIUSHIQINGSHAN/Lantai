...
| **瞭望**（Liaowang，后台监控面板） | 悬镜控制台内的系统运维监控面板正式名（ADR-0013 意象「瞭望」= 古代瞭望塔登高望远、侦候军情）：把依赖健康（SQLite/ChromaDB/LLM）、人工闸门队列水位、Worker 调度漏跑、零召回率与检索延迟、吞吐双序列、摄取任务成败、磁盘占用与功能开关聚合为只读快照（`GET /monitor/snapshot`，纯函数 `lantai/ops/monitor.py`），30 秒自动刷新；只读不改写任何系统状态，Worker 手动处置仍走案牍通道。 |
