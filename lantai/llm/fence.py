...
进入下游 LLM 提示的记忆正文，统一以 ``<memory_data>`` 围栏包裹并附固定声明。
单一真源：各出口（shell_hook / MCP search / 认知中间件 / to_prompt /
reflector 内部读）一律 import 本模块，不得各自拼围栏。
...
def neutralize_fence_escapes(content: str) -> str:
    """防逃逸：正文中出现的闭合标记中性化，保证围栏不可被正文截断。

    正文其余内容原样保留（含「忽略以上指令」类文本——它们仍在数据位内，
    由声明行约束下游 LLM 的解读方式）。
    """
    return (content or "").replace(DATA_FENCE_CLOSE, _DATA_FENCE_CLOSE_ESCAPED)
