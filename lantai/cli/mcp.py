...
def handle_recall_report(params: dict) -> dict:
    ...
    if days is not None and (
        not isinstance(days, int) or isinstance(days, bool) or not (1 <= days <= 365)
    ):
...
def handle_mem_health(params: dict) -> dict:
    ...
    try:
        ...
        with db.get_session() as s:
            checks["sqlite"] = "ok"
            checks["memory_count"] = int(s.exec(select(func.count()).select_from(MemoryItem)).one())
...
def handle_autodream_report(params: dict) -> dict:
    ...
    if limit is not None and (
        not isinstance(limit, int) or isinstance(limit, bool) or not (1 <= limit <= 5000)
    ):
...
TOOLS = {
    "search": {
        "description": "搜索记忆（支持辨域 domain 过滤）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "graph_expand_search": {
        "description": "贯珠：图增强检索（混合初筛 + 沿 MemoryEdge 展开 1~2 步拓扑二度联想，ADR-0035）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "add": {
        "description": "添加记忆（media_url 提供时走目识 vision：图片 -> 视觉描述作为正文，与 content 二选一）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "title": {"type": "string"},
                ...
                "media_url": {
                    "type": "string",
                    "description": "目识（vision）：图片地址（http/https）或 data URI；提供时 content 必须为空",
                },
    ...
    "backfill": {
        "description": "回填弱标注：回答时实际用到的记忆 id 写回检索事件",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "add_dialogue": {
        "description": "对话写通道：提交对话文本，自动提炼候选记忆（记住直通/闲聊入队）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "dialogue_task_status": {
        "description": "潜移：查询异步对话摄取任务状态与提纯结果（ADR-0033）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "candidates_pending": {
        "description": "列出待审候选（被闸门拒绝、等人工裁决）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "candidate_review": {
        "description": "审核候选：approve 仅生成 pending 提案（最终写入需再批准提案），reject 归档",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "candidate_refine": {
        "description": "披沙（精炼）：对指定候选记忆执行指代消解与结构化提纯（ADR-0030）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "triage_analyze": {
        "description": "持节（智能研判）：扫描待审候选库，由 AI 批量研判并生成决策建议（approve/reject/refine/manual）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "triage_apply": {
        "description": "持节（批量审批）：批量采纳并执行研判决策（批准入提案、淘汰归档、精炼提纯）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "actions": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "action": {
                                "type": "string",
                                "description": "approve | reject | refine",
                            },
    ...
    "triage_auto_pilot": {
        "description": "持节（一键自治）：智能体案牍巡检官一键全自动巡检（扫描→提纯→淘汰噪音→批准优质事实）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "min_approve_conf": {
                    "type": "number",
                    "default": 0.85,
                    "description": "自动批准的置信度门槛 [0.0, 1.0]",
                },
                "max_reject_conf": {
                    "type": "number",
                    "default": 0.25,
                    "description": "自动淘汰的置信度天花板 [0.0, 1.0]",
                },
                "limit": {"type": "integer", "default": 50, "description": "巡检候选条数 [1,500]"},
                "dry_run": {
                    "type": "boolean",
                    "default": False,
                    "description": "true=仅预演生成报告不写库",
                },
            },
        },
    },
    "kaogong_eval": {
        "description": "考功（演化）：基于长程使用反馈与采纳率，对全库记忆执行功过升降级评定（ADR-0031）",
        "inputSchema": {
            "type": "object",
            "properties": {},
        },
    ...
    "memory_consolidate": {
        "description": "沉潜（折叠压缩）：扫描高密度碎片记忆聚类提纯为主记忆，并修剪极度衰减的边缘噪音（ADR-0036）",
        "inputSchema": {
            "type": "object",
            "properties": {},
        },
    ...
    "consolidation_report": {
        "description": "沉潜（审计报告）：获取最近一次夜梦沉淀与折叠压缩的审计结果（ADR-0036）",
        "inputSchema": {
            "type": "object",
            "properties": {},
        },
    ...
    "probe_detect": {
        "description": "探颐（主动探针）：检测当前检索查询相关的未决冲突与模糊事实探针（ADR-0037）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "probe_resolve": {
        "description": "探颐（消歧闭环）：根据用户次轮自然答复自动消解冲突与更替记忆版本（ADR-0037）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "raw_add": {
        "description": "原文直存（verbatim）：内容直入 FTS5+向量，零 LLM",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "obsidian_sync": {
        "description": "Obsidian 笔记同步：原文直存 + [[双链]] 实体/边沉淀（幂等）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "title": {"type": "string", "default": ""},
    ...
    "rollback": {
        "description": "回滚记忆到上一版本（Checkpoint 快照）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "memory_id": {"type": "string"},
    ...
    "conflict_resolve": {
        "description": "裁决冲突事件：resolved / dismissed",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "recall_report": {
        "description": "零召回率监控报告：最近 N 天检索聚合（零召回率/按 lane/场景命中/token 成本）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "scene_get": {
        "description": "下钻场景：返回场景元数据与全部成员详情（渐进式披露）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "scene_id": {
                    "type": "string",
                    "description": "场景 id（导航块或 scenes_list 中可见）",
                },
            },
            "required": ["scene_id"],
        },
    ...
    "scenes_list": {
        "description": "列出场景（按热度排序），供浏览可用场景",
        "inputSchema": {
            "type": "object",
            "properties": {
                "limit": {"type": "integer", "default": 50},
    ...
    "get_digest": {
        "description": "获取今日记忆盘点报告（摘要 + 五项统计）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "mem_help": {
        "description": "mem:help——返回支持的 mem: 命令表与示例（命令式维护入口）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "mem_sync": {
        "description": "mem:sync——刷新注入资产：场景增量聚类补跑 + 今日 digest 重算",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "mem_create_skill": {
        "description": "mem:create-skill——沉淀 Skill 资产（名称 + 描述 + 步骤，procedural 永不衰减）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "offload_read": {
        "description": "读取卸载全文：长记忆经上下文卸载（摘要+路径注入）后，按 memory_id 取完整原文",
        "inputSchema": {
            "type": "object",
            "properties": {
                "memory_id": {
                    "type": "string",
                    "description": "记忆 id（卸载注入行或 evidence 中可见）",
                },
            },
            "required": ["memory_id"],
        },
    ...
    "wiki_read": {
        "description": "读取记忆 Wiki 页：index/overview 列出的页面按 slug 下钻取正文（wikilink 下钻）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "slug": {
                    "type": "string",
                    "description": "页面 slug（index.md 链接或 overview [[wikilink]] 中可见）",
                },
            },
            "required": ["slug"],
        },
    ...
    "mem_recent": {
        "description": "最近记忆（只读）：按更新时间倒序列出 active 记忆",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "mem_stats": {
        "description": "记忆概览（只读聚合）：总数/按 lane/decay_class/待审候选积压/检查点/待审提案",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "mem_health": {
        "description": "深度健康检查：SQLite 可读 + 向量存储可用（不触发外部 LLM 调用）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "autodream_report": {
        "description": "蒸馏预演（dry-run 不写库）：聚类 → 规划，返回将产出的提案计划与跳过清单",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "autodream_trigger": {
        "description": "执行一轮蒸馏：聚类 → 规划 → 落 pending 提案（低置信度进 skipped，人工裁决后才应用）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "proposals_list": {
        "description": "待审提案列表（蒸馏/反射产出，等人工裁决）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "status": {
    ...
    "proposal_decide": {
        "description": "裁决提案：approve 应用（先落 Checkpoint 可回滚），reject 归档并记 reason",
        "inputSchema": {
            "type": "object",
            "properties": {
                "proposal_id": {
                    "type": "string",
                    "description": "提案 id（proposals_list 中可见）",
                },
                "approve": {"type": "boolean", "description": "true=应用；false=拒绝归档"},
                "reason": {
                    "type": "string",
                    "default": "",
                    "description": "裁决理由（落库可审计）",
                },
            },
    ...
    "tree_view": {
        "description": "分类树视图（只读）：节点 + 每节点挂载计数",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "tree_add": {
        "description": "新增分类树节点（父缺失/重名/非法名 -> 校验失败，宁 miss 不脏写）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "tree_assign": {
        "description": "把记忆挂到分类树节点（节点/记忆必须存在）",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "node_path": {
                    "type": "string",
                    "description": "目标节点路径，如 /projects/release",
                },
            },
    ...
    "crystals_list": {
        "description": "结晶候选项列表（默认 candidate 待审）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "status": {
                    "type": "string",
                    "default": "candidate",
                    "description": "candidate|approved|archived",
                },
                "limit": {"type": "integer", "default": 50},
            },
        },
    ...
    "crystals_detect": {
        "description": "执行一轮结晶检测：聚类 -> 候选（dry_run=true 不写库；噪声 lane 排除）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "crystal_decide": {
        "description": "裁决结晶候选：approve 必须带非空 steps -> 落成 Skill 资产；reject -> archived",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "approve": {
                    "type": "boolean",
                    "description": "true=批准（需 steps）；false=拒绝归档",
                },
                "steps": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "批准时必填：Skill 执行步骤",
                },
                "reason": {"type": "string", "default": ""},
            },
    ...
    "reflect_run": {
        "description": "执行一轮反思：健康扫描 -> 提案 -> 裁决（高置信 auto-apply，中风险落 pending，宁 miss 不脏写）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "mem_usage": {
        "description": "用量统计（只读）：最近 N 天每日新增记忆数（缺日补零）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "core_memory_get": {
        "description": "读取核心记忆块（只读）：identity/task/policy 持久块列表",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "verbatim_search": {
        "description": "原文直存检索（verbatim 专用通道）：原文默认不进混合召回，此通道可查（FTS+向量）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "搜索查询"},
    ...
    "graph_view": {
        "description": "记忆关系星图（只读）：节点 + MemoryEdge 链接（supports/refines/contradicts/supersedes）+ lane/relation 统计",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "recall_chain": {
        "description": "记忆广播链（烽燧，只读）：seed 记忆逐层触发关联记忆",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "min_score": {
                    "type": "number",
                    "default": 0.3,
                    "description": "入选最低分数 [0,1]",
                },
                "total_max": {
                    "type": "integer",
                    "default": 20,
                    "description": "链总记忆上限 [1,50]",
                },
            },
            "required": ["q"],
        },
    ...
    "checkpoint_write": {
        "description": "底本：写入五段会话快照（在做/下一步/工作区/决策/待办），下次会话启动注入",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "checkpoint_latest": {
        "description": "底本：读取最近一次会话快照（只读）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "persona_get": {
        "description": "器识：获取当前激活人格基座（L/G/E 三层与提示块，只读）",
        "inputSchema": {"type": "object", "properties": {}},
    },
    "persona_set": {
        "description": "器识：设置/更新人格基座（L/G/E 三层认知模型）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "scratchpad_get": {
        "description": "札记：读取指定会话的工作区便签（ADR-0032，只读）",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    "scratchpad_write": {
        "description": "札记：更新/覆盖指定会话的工作区便签（ADR-0032，最大 1000 字符）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "session_id": {"type": "string", "default": "default", "description": "会话标识"},
...
def handle(msg: dict) -> dict | None:
    ...
    if method == "tools/list":
        return {
            "jsonrpc": "2.0",
            "id": mid,
            "result": {"tools": [{"name": n, **meta} for n, meta in TOOLS.items()]},
        }
