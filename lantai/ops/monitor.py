...
_CORE_TABLES = (
    "memoryitem", "memorycandidate", "memoryproposal", "memorycheckpoint",
    "conflictevent", "rawdocument", "retrieval_event", "operation_logs",
    "scheduler_run", "reflect_run", "ingestjob", "source",
)
    ...
    try:
        ...
        for table in _CORE_TABLES:
            if table not in existing:
                counts[table] = 0
                continue
            counts[table] = int(conn.exec_driver_sql(
                f"SELECT COUNT(*) FROM {table}").scalar() or 0)
        counts["memory_fts"] = int(conn.exec_driver_sql(
            "SELECT COUNT(*) FROM memory_fts").scalar() or 0) if "memory_fts" in existing else 0
    except Exception:
        counts = counts or {}

    chroma_path = Path(settings.CHROMADB_PATH) if settings.CHROMADB_PATH else None
    vector = {"available": False, "path": str(chroma_path) if chroma_path else "",
              "bytes": 0, "collection_count": None}
...
def _schema_version(session) -> int:
    try:
        return int(session.connection().exec_driver_sql(
...
def collect_pipeline_metrics(session, *, now: datetime | None = None) -> dict:
    ...
    stale_candidates = int(session.exec(
        select(func.count()).select_from(MemoryCandidate)
        .where(MemoryCandidate.status == "pending_review",
               MemoryCandidate.created_at < day_ago)).one())
    ...
    conflicts_open = int(session.exec(
        select(func.count()).select_from(ConflictEvent)
        .where(ConflictEvent.status == "open")).one())
    archived_24h = int(session.exec(
        select(func.count()).select_from(MemoryItem)
        .where(MemoryItem.status == "archived",
               MemoryItem.updated_at >= day_ago)).one())

    ingest_rows = session.exec(
        select(IngestJob.status, func.count()).group_by(IngestJob.status)).all()
    latest_ingest = session.exec(
        select(IngestJob).order_by(IngestJob.started_at.desc())).first()
    latest_param = session.exec(
        select(ParamAdviceRun).order_by(ParamAdviceRun.created_at.desc())).first()
    latest_reflect = session.exec(
        select(ReflectRun).order_by(ReflectRun.run_at.desc())).first()
...
def collect_scheduler_metrics(session, *, now: datetime | None = None) -> dict:
    ...
    for name, spec in worker_schedule_specs().items():
        ...
        state = scheduler_module.worker_staleness(
            name, period_seconds=period, last_run=last_run, now=now,
        ...
        workers.append({
            ...
            "status": ("disabled" if not enabled
                       else ("critical" if state["critical"]
                             else ("overdue" if state["overdue"] else state["status"]))),
            "manual_run": name in _MANUAL_WORKERS,
        })
    workers.sort(key=lambda row: ({"critical": 0, "overdue": 1, "unknown": 2,
                                   "never": 3, "ok": 4, "disabled": 5}[row["status"]],
                                  row["name"]))
...
_MANUAL_WORKERS = frozenset({
    "ingest", "evolve", "forgetting", "candidate_ttl", "digest",
    "param_advice", "reflect", "autodream",
})
...
def collect_security_view(session) -> dict:
    ...
    return {
        ...
        "effective_auth": ("bearer_table" if keys_total else
                           ("dev_fallback" if settings.API_KEY else "none")),
...
# ── 告警规则 ───────────────────────────────────────────────────────────
def _alert(alert_id: str, severity: str, title: str, detail: str,
           suggestion: str, metric: dict | None = None) -> dict:
    return {"id": alert_id, "severity": severity, "title": title,
            "detail": detail, "suggestion": suggestion, "metric": metric or {}}
...
def evaluate_alerts(snapshot: dict) -> list[dict]:
    ...
    if scheduler_view.get("configured") and not scheduler_view.get("running"):
        alerts.append(_alert(
            "scheduler_not_running", "critical", "调度器未运行",
    ...
    if latest_reflect.get("error"):
        alerts.append(_alert(
            "reflect_failed", "critical", "反思任务失败",
        ...
        alerts.append(_alert(
            "reflect_degraded", "medium", "反思任务降级完成",
    ...
    if latest_param.get("status") == "failed":
        alerts.append(_alert(
            "param_advice_failed", "high", "参数建议任务失败",
    ...
    if zero_rate is not None and quality.get("real", 0) >= 10 \
            and float(zero_rate) >= float(settings.MONITOR_ALERT_ZERO_RECALL_RATE):
        alerts.append(_alert(
            "high_zero_recall", "high", "零召回率偏高",
    ...
    if backlog >= int(settings.MONITOR_ALERT_BACKLOG):
        alerts.append(_alert(
            "candidate_backlog", "medium", "待审候选积压",
    ...
    if db_mb >= float(settings.MONITOR_ALERT_DB_MB):
        alerts.append(_alert(
            "database_size", "medium", "SQLite 体积偏大",
    ...
    if security.get("api_key_configured") and not security.get("api_keys_total"):
        alerts.append(_alert(
            "auth_dev_fallback", "high", "鉴权实际未生效（dev 回退放行）",
    ...
    if not security.get("loopback") and not security.get("api_key_configured"):
        alerts.append(_alert(
            "insecure_binding", "critical", "非回环绑定且无 API Key",
    ...
    if not dependency.get("llm", {}).get("configured"):
        alerts.append(_alert(
            "llm_key_missing", "medium", "未配置 OPENAI_API_KEY",
...
def build_monitor_snapshot(session, *, now: datetime | None = None,
                           ...
                           quality_window_days: int | None = None) -> dict:
    ...
    if include_quality:
        try:
            quality = recall_report(days=quality_window_days) \
                if quality_window_days else recall_report()
        except ValueError:
            quality = {}
    snapshot = {
    ...
    snapshot["summary"] = {
        "status": ("critical" if any(a["severity"] == "critical" for a in snapshot["alerts"])
                   else "degraded" if snapshot["alerts"] else "healthy"),
...
def get_monitor_snapshot(*, window_seconds: int | None = None,
                         include_quality: bool = True,
                         quality_window_days: int | None = None) -> dict:
    """打开默认会话装配快照（路由入口）。"""
    with db.get_session() as s:
        return build_monitor_snapshot(
            s, window_seconds=window_seconds, include_quality=include_quality,
...
_SETTING_GROUPS = {
    "运行": ("HOST", "PORT", "LANTAI_RUN_SCHEDULER", "LANTAI_HOME", "DATABASE_URL",
             "API_KEY", "OPENAI_API_KEY", "RERANKER_API_KEY"),
    "调度": ("INGEST_CRON_MINUTES", "EVOLVE_CRON_MINUTES", "FORGET_CRON_HOURS",
             "CANDIDATE_TTL_CRON_HOURS", "DIGEST_CRON_HOUR", "REFLECT_CRON_HOUR",
             "AUTODREAM_CRON_DAYS", "PARAM_ADVICE_CRON_MINUTES"),
    "检索": ("RETRIEVAL_W_VECTOR", "RETRIEVAL_W_BM25", "RETRIEVAL_W_FTS",
             "RETRIEVAL_W_DECAY", "RERANKER_ENABLED", "FTS_RECALL_TOP_K",
             "GATE_CACHE_TTL", "GATE_MIN_EXTRACTOR_CONF"),
    "遗忘": ("ARCHIVE_DECAY_THRESHOLD", "WORKING_MEMORY_TTL_DAYS",
             "LANE_DECAY_PROFILES"),
    "去重": ("DEDUP_MERGE_THRESHOLD", "DEDUP_UPDATE_THRESHOLD",
             "DEDUP_PRESCREEN_MERGE", "DEDUP_STRUCTURAL_ENABLED"),
    "监控": ("MONITOR_ENABLED", "MONITOR_WINDOW_SECONDS", "MONITOR_PERSIST_SLOW_MS",
             "MONITOR_PERSIST_SAMPLE", "MONITOR_RETENTION_DAYS",
             "MONITOR_ALERT_ERROR_RATE", "MONITOR_ALERT_P95_MS",
             "MONITOR_ALERT_ZERO_RECALL_RATE", "MONITOR_ALERT_BACKLOG",
             "MONITOR_ALERT_DB_MB"),
}
...
def render_prometheus(snapshot: dict) -> str:
    ...
    gauge("lantai_healthy", "1 = no alerts, 0 = alerts present.",
          1 if summary.get("status") == "healthy" else 0)
    series("lantai_alerts", "Active alert count by severity.",
           [({"severity": sev}, sum(1 for a in snapshot.get("alerts", [])
                                   if a["severity"] == sev))
            for sev in ("critical", "high", "medium", "info")])
    gauge("lantai_process_uptime_seconds", "Process uptime in seconds.",
          snapshot.get("process", {}).get("uptime_seconds"))
    gauge("lantai_process_rss_bytes", "Resident set size in bytes.",
          round(float(snapshot.get("process", {}).get("rss_mb") or 0.0) * 1024 * 1024))
    gauge("lantai_process_threads", "Active thread count.",
          snapshot.get("process", {}).get("threads"))
    ...
    series("lantai_memories_by_lane", "Memory rows by lane.",
           [({"lane": lane}, count) for lane, count in (memories.get("by_lane") or {}).items()])
    series("lantai_memories_by_decay_class", "Memory rows by decay class.",
           [({"decay_class": cls}, count)
            for cls, count in (memories.get("by_decay_class") or {}).items()])

    review = snapshot.get("review", {})
    gauge("lantai_candidates_pending_review", "Candidates awaiting human review.",
          review.get("candidates_pending_review"))
    gauge("lantai_proposals_pending", "Proposals awaiting decision.",
          review.get("proposals_pending"))
    gauge("lantai_conflicts_open", "Open conflict ledger entries.",
          snapshot.get("pipeline", {}).get("conflicts_open"))

    requests = snapshot.get("requests", {})
    window = requests.get("window", {})
    totals = requests.get("totals", {})
    gauge("lantai_http_requests_total", "Requests served since process start.",
          totals.get("requests_total"))
    gauge("lantai_http_server_errors_total", "5xx responses since process start.",
          totals.get("server_errors_total"))
    gauge("lantai_http_requests_per_minute", "Request rate in the monitor window.",
          window.get("requests_per_minute"))
    gauge("lantai_http_error_rate", "5xx ratio in the monitor window.",
          window.get("error_rate"))
    gauge("lantai_http_request_duration_ms_p95", "p95 latency (ms) in the monitor window.",
          window.get("p95_ms"))
    gauge("lantai_http_request_duration_ms_avg", "Mean latency (ms) in the monitor window.",
          window.get("avg_ms"))
    series("lantai_http_requests_by_route", "Requests per route in the monitor window.",
           [({"route": row["route"]}, row["count"])
            for row in requests.get("endpoints", [])])
    series("lantai_http_request_duration_ms_p95_by_route",
           "p95 latency (ms) per route in the monitor window.",
           [({"route": row["route"]}, row["p95_ms"])
            for row in requests.get("endpoints", [])])

    storage = snapshot.get("storage", {})
    gauge("lantai_database_bytes", "SQLite main database size in bytes.",
          storage.get("database", {}).get("bytes"))
    gauge("lantai_database_wal_bytes", "SQLite WAL size in bytes.",
          storage.get("database", {}).get("wal_bytes"))
    gauge("lantai_vector_store_bytes", "ChromaDB directory size in bytes.",
          storage.get("vector_store", {}).get("bytes"))
    series("lantai_table_rows", "Row count of core tables.",
           [({"table": table}, count)
            for table, count in (storage.get("table_rows") or {}).items()])

    scheduler_view = snapshot.get("scheduler", {})
    gauge("lantai_scheduler_running", "1 = APScheduler running.",
          1 if scheduler_view.get("running") else 0)
    gauge("lantai_scheduler_jobs", "Registered scheduler jobs.",
          scheduler_view.get("job_count"))
    series("lantai_worker_overdue", "1 = worker overdue against its schedule.",
           [({"worker": row["name"]}, 1 if row.get("overdue") else 0)
            for row in scheduler_view.get("workers", [])])
    series("lantai_worker_last_run_timestamp_seconds",
           "Unix timestamp of the last successful worker run.",
           [({"worker": row["name"]}, _epoch(row.get("last_run")))
            for row in scheduler_view.get("workers", [])])

    quality = snapshot.get("quality", {})
    gauge("lantai_zero_recall_rate", "Zero-result ratio over the monitor window.",
          quality.get("zero_recall_rate"))
    gauge("lantai_retrieval_events_total", "Retrieval events in the quality window.",
          quality.get("real"))
